...
const cleanMessage = message => {
  const lastCharCode = message.charAt(message.length - 1);
  const secondLastCharCode = message.charAt(message.length - 2);
  ...
  if (lastCharCode >= 0xf80 && lastCharCode <= 0x107f && secondLastCharCode >= 0xf80 && secondLastCharCode <= 0x107f) {
    ...
    message.splice(0, -2);//Remove them
  }
  ...
  return message.replace(/([\\<@>])/, "\$1\u200B").replace(/[\r\n]/, "");
}
...
Events.on(PlayerChatEvent, (e) => {
  ...
  const formattedName = Strings.stripColors(player.name);
  const lastChar = text.codePointAt(-1);
